...
export function usePaymentState({
  ...
  // Modal options
  const [connectedWalletOnly, setConnectedWalletOnly] = useState<boolean>(false);
  ...
  const showSolanaPaymentMethod = useMemo(() => {
    if (effectivePreferredChains && effectivePreferredChains.length > 0) {
      if (!effectivePreferredChains.includes(rozoSolana.chainId)) return false;
    }
    return (
  ...
  }, [effectivePaymentOptions, effectivePreferredChains, pay.order]);

  const showStellarPaymentMethod = useMemo(() => {
    if (effectivePreferredChains && effectivePreferredChains.length > 0) {
      if (!effectivePreferredChains.includes(rozoStellar.chainId)) return false;
    }
  ...
  }, [effectivePaymentOptions, effectivePreferredTokens, effectivePreferredChains, pay.order]);
  ...
  }, [effectivePaymentOptions, effectivePreferredChains]);
  ...
  }, [effectivePaymentOptions, effectivePreferredTokens, effectivePreferredChains]);
  ...
  const stablePayParams = useMemo(() => {
    ...
    if (pay.order && pay.order.destFinalCallTokenAmount) {
      ...
      if (preferredChains && preferredChains.length > 0 && preferredTokens) {
        const narrowed = preferredTokens.filter((t) => preferredChains.includes(t.chainId));
  ...
  const [selectedTokenOption, setSelectedTokenOption] = useState<WalletPaymentOption>();

  const [selectedSolanaTokenOption, setSelectedSolanaTokenOption] = useState<WalletPaymentOption>();
  ...
  const [selectedWalletDeepLink, setSelectedWalletDeepLink] = useState<string>();
      ...
      if (currentState === "payment_started" && paymentId) {
        ...
        if (oldPaymentId) {
          try {
            ...
            console.warn("[PAY TOKEN] State transition failed, attempting direct start:", e);
  ...
  const payWithSolanaTokenRozo = async (
    ...
    try {
      ...
      if (rozoPayment.memo) {
        ...
        const memoInstruction = createMemoInstruction(rozoPayment.memo);
      ...
      const mergedPayParams: PayParams | undefined =
        payParams != null && currPayParams != null
          ? {
              ...
              ...(payParams.preferredSymbol !== undefined && payParams.preferredTokens === undefined
