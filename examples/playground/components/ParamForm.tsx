...
export function ParamForm({
  ...
  const addressPlaceholder = useMemo(() => {
    ...
    switch (selectedChain.type) {
      case "solana": return "e.g. 9no8…Bzr (Solana address)";
      case "stellar": return "e.g. GABC…XYZ (Stellar G-address)";
      default: return "e.g. 0x1234…abcd (EVM address)";
    }
  }, [selectedChain]);
    ...
    <div className="space-y-4">
              ...
              {selectedChain && (
                <span className="flex items-center gap-2">
                  {selectedChain.LogoComponent && (
                    <selectedChain.LogoComponent width={18} height={18} className="shrink-0 rounded-full" />
                  )}
                  {selectedChain.name}
                </span>
              )}
          ...
          <SelectContent>
            {chains.map((c) => (
              <SelectItem key={c.chainId} value={String(c.chainId)}>
                <span className="flex items-center gap-2">
                  {c.LogoComponent && (
                    <c.LogoComponent width={18} height={18} className="shrink-0 rounded-full" />
                  )}
                  {c.name}
                </span>
              </SelectItem>
      ...
      <div className="space-y-1.5">
        ...
        <Input
          ...
          className={`bg-secondary border-border font-mono text-xs ${addressError ? "border-destructive focus-visible:ring-destructive" : ""}`}
        />
        {addressError && (
          <p className="text-[11px] text-destructive leading-tight">{addressError}</p>
        )}
      </div>
